...
@app.route('/admin/study-type-management')
def study_type_management():
...
@app.route('/admin/study-type-management/add', methods=['POST'])
def study_type_management_add():
    ...
    study_type_data = {
        'descripcion_tipo_estudio': data.get('descripcion_tipo_estudio')
    }

    Study_type.create(study_type_data)

    return jsonify({'status': 'success', 'message': 'Tipo de estudio creado correctamente'}), 200

@app.route('/admin/study-type-management/update', methods=['PATCH'])
def study_type_management_update():
