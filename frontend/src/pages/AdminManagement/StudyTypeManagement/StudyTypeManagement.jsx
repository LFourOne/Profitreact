...
import styles from './StudyTypeManagement.module.css';
...
export function StudyTypeManagement() {
    ...
    const fetchApi = async () => {
        try {
            const response = await apiClient.get('/admin/study-type-management');
    ...
    const handleToggleEdit = (studyType) => {
        setToggleEdit(!toggleEdit);
        if (toggleEdit) {
            setSelectedStudyType(null);
            setValueEdit("descripcion_tipo_estudio", "");
            setValueEdit("id_tipo_estudio", "");
        } else {
            setSelectedStudyType(studyType);
            setValueEdit("descripcion_tipo_estudio", studyType.descripcion_tipo_estudio);
            setValueEdit("id_tipo_estudio", studyType.id_tipo_estudio);
        }
    }
    ...
    const onSubmitAdd = async (data) => {
        ...
        const confirmed = window.confirm('¿Estás seguro de que deseas crear un nuevo tipo de estudio?');
        if (!confirmed) {
            return;
        }

        try {
            const response = await apiClient.post('/admin/study-type-management/add', data);
            resetAdd();
            setShowModal(false);
            fetchApi();
        } catch (error) {
    ...
    const onSubmitEdit = async (data) => {
        ...
        const confirmed = window.confirm('¿Estás seguro de que deseas actualizar el tipo de estudio?');
        if (!confirmed) {
            return;
        }

        try {
            const response = await apiClient.patch('/admin/study-type-management/update', data);
            resetEdit();
            setToggleEdit(false);
            fetchApi();
        } catch (error) {
            ...
            <main className={styles['main']}>
                <section className={styles['header-section']}>
                    <div className={styles['header-section-text-container']}>
                        <h1>Mantenedor de Tipos de Estudio</h1>
                        <p>Administra y gestiona todos los tipos de estudio de Profit</p>
                    </div>
                            ...
                            <tbody>
                                {
                                    studyTypes.map((studyType) => (
                                        <tr key={studyType.id_tipo_estudio}>
                                            {
                                                toggleEdit && studyType.id_tipo_estudio === selectedStudyType.id_tipo_estudio ? (
                                                    <td>
                                                        <form onSubmit={handleSubmitEdit(onSubmitEdit)} id='update-form'>
                                                            <input type="text" {...registerEdit("descripcion_tipo_estudio")} />
                                                            <input type="hidden" value={selectedStudyType.id_tipo_estudio} {...registerEdit("id_tipo_estudio")} />
                                                        </form>
                                                    </td>
                                                ) : (
                                                    <td>{studyType.descripcion_tipo_estudio}</td>
                                                )
                                            }
                                            <td>
                                                <div>
                                                    ...
                                                    {
                                                        toggleEdit && studyType.id_tipo_estudio === selectedStudyType.id_tipo_estudio ? (
                                                            <button onClick={() => handleToggleEdit(studyType)}>
                    ...
                    showModal && (
                        <div className={styles['modal-overlay']} onClick={handleCloseModal}>
                            <div className={styles['modal']} onClick={(e) => e.stopPropagation()}>
                                <header>
                                    <h1>Crear tipo de estudio</h1>
                                </header>
                                <form onSubmit={handleSubmitAdd(onSubmitAdd)} id='add-form'>
                                    <fieldset>
                                        <label>Nombre del tipo de estudio</label>
                                        <input type="text" {...registerAdd('descripcion_tipo_estudio', {required: true})} />
                                    </fieldset>
                                </form>
                                <footer>
