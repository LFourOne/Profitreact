...
export function PositionManagement() {
    ...
    const fetchApi = async () => {
        try {
            const response = await apiClient.get('/admin/position-management');
    ...
    const handleToggleEdit = (position) => {
        setToggleEdit(!toggleEdit);
        if (toggleEdit) {
            setSelectedPosition(null);
            setValueEdit("cargo", "")
            setValueEdit("id_cargo", "")
        } else {
            setSelectedPosition(position);
            setValueEdit("cargo", position.cargo);
            setValueEdit("id_cargo", position.id_cargo);
        }
    }
    ...
    const onSubmitAdd = async (data) => {
        ...
        const confirmed = window.confirm('¿Estás seguro de que deseas crear un nuevo cargo?');
        if (!confirmed) {
            return;
        }

        try {
            const response = await apiClient.post('/admin/position-management/add', data);
            resetAdd();
            setShowModal(false);
            fetchApi();
        } catch (error) {
    ...
    const onSubmitEdit = async (data) => {
        ...
        const confirmed = window.confirm('¿Estás seguro de que deseas actualizar el cargo?');
        if (!confirmed) {
            return;
        }

        try {
            const response = await apiClient.patch('/admin/position-management/update', data);
            resetEdit();
            setToggleEdit(false);
            fetchApi();
        } catch (error) {
            ...
            <main className={styles['main']}>
                <section className={styles['header-section']}>
                    <div className={styles['header-section-text-container']}>
                        ...
                        <p>Administra y gestiona todos los cargos de Profit</p>
                    </div>
                            ...
                            <tbody>
                                {
                                    positions.map((position) => (
                                        <tr key={position.id_cargo}>
                                            {
                                                toggleEdit && position.id_cargo === selectedPosition.id_cargo ? (
                                                    <td>
                                                        <form onSubmit={handleSubmitEdit(onSubmitEdit)} id='update-form'>
                                                            <input type="text" {...registerEdit("cargo")} />
                                                            <input type="hidden" value={selectedPosition.id_cargo} {...registerEdit("id_cargo")} />
                                                        </form>
                                                    </td>
                                                ) : (
                                            ...
                                            <td>
                                                <div>
                                                    ...
                                                    {
                                                        toggleEdit && position.id_cargo === selectedPosition.id_cargo ? (
                                                            <button onClick={() => handleToggleEdit(position)}>
                            ...
                            <div className={styles['modal']} onClick={(e) => e.stopPropagation()}>
                                ...
                                <form onSubmit={handleSubmitAdd(onSubmitAdd)} id='add-form'>
                                    <fieldset>
                                        ...
                                        <input type="text" {...registerAdd('cargo', {required: true})} />
                                    </fieldset>
                                </form>
                                <footer>
